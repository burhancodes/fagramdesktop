...
void RemoveQuarantineFromBundle(NSString *path) {
	...
	RemoveQuarantineAttribute([path stringByAppendingString:@"/Contents/MacOS/FAgram"]);
...
int main(int argc, const char * argv[]) {
	...
	if (update) {
		...
		// Take the Updater (this currently running binary) from the place where it was placed by FAgram
		// and copy it to the folder with the new version of the app (ready),
		// so it won't be deleted when we will clear the "FAgram.app/Contents" folder.
